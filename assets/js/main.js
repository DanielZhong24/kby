...
(function ($) {
    ...
    /*==============================
        16. Dynamic detail page
    ================================*/
	document.addEventListener('DOMContentLoaded', function() {
		document.body.addEventListener('click', function(event) {
			const productLink = event.target.closest('.product-link')
			// Check if the clicked element has the product-link class
			if (productLink) {
				event.preventDefault();

				// Pass the clicked element to the redirectToDetails function
				redirectToDetails(productLink);
				console.log("hi");
	...
	function redirectToDetails(clickedElement) {
		// Find the relevant information within the clicked element
		const productName = clickedElement.closest('.product').querySelector('.productName').innerText;
		const price = clickedElement.closest('.product').querySelector('.prize').innerText;
	    const imageSrc = clickedElement.closest('.product').querySelector('.thumbnail img').getAttribute('src');
		// Encode the variables and redirect to the details page
		const encodedProductName = encodeURIComponent(productName);
		const encodedPrice = encodeURIComponent(price);
		const encodedImageSrc = encodeURIComponent(imageSrc);
		
		window.location.href = `product-details.html?productName=${encodedProductName}&price=${encodedPrice}&imageSrc=${encodedImageSrc}`;
	}
